...
TRAIN_RUN_NAME = "fire_yolo11n_tuned"
TEST_RUN_NAME = "fire_yolo11n_tuned_test"
...
# 기본 구조:
# 현재 Python 파일 위치/
# └── runs/data/fire_yolo11n/weights/best.pt
#
# 실제 best.pt 위치가 다르면 아래 경로만 수정하세요.
# ------------------------------------------------------------

PRETRAINED_BEST_MODEL = (
    BASE_DIR
    / "runs"
    / "data"
    / "fire_yolo11n"
...
def get_dataset_paths():
    ...
    if not isinstance(yaml_data, dict):
        ...
        raise ValueError(
            "data.yaml의 최상위 내용은 key-value 형식이어야 합니다."
        )
    ...
    if isinstance(yaml_names, dict):
        ...
        try:
            yaml_names = [yaml_names[index] for index in range(len(yaml_names))]
        except (KeyError, TypeError):
            raise ValueError(
                "data.yaml의 names 딕셔너리는 0부터 연속된 정수 key를 사용해야 합니다."
            )

    if yaml_names != CLASS_NAMES:

        raise ValueError(
            "data.yaml의 클래스 정의가 코드와 다릅니다.\n"
            f"data.yaml names: {yaml_names}\n"
            f"코드 CLASS_NAMES: {CLASS_NAMES}"
        )

    yaml_nc = yaml_data.get("nc")

    if yaml_nc is not None and yaml_nc != len(CLASS_NAMES):

        raise ValueError(
            f"data.yaml nc={yaml_nc}, 코드 클래스 수={len(CLASS_NAMES)}가 다릅니다."
        )
    ...
    for split_name, split_value in split_values.items():
        ...
        if not isinstance(split_value, (str, Path)):
            ...
            raise TypeError(
                f"data.yaml의 {split_name}은 단일 디렉터리 경로여야 합니다. "
                f"현재 형식: {type(split_value).__name__}. "
                "여러 경로/list/txt/URL을 사용하려면 통계 코드도 해당 형식에 맞게 확장해야 합니다."
            )
...
def match_predictions(
    ...
    # 클래스가 다른 박스끼리 서로 매칭하지 않습니다.
    # 증강 경로(augmenting path)를 사용해 greedy 매칭이 유효한
    # 두 개의 매칭을 하나로 줄이는 경우를 방지합니다.
    adjacency = {}
    iou_lookup = {}
    ...
    for actual_index, actual in enumerate(actual_boxes):
        ...
        for predicted_index, predicted in enumerate(predicted_boxes):
            ...
            if int(actual[0]) != int(predicted[0]):
                continue

            iou = box_iou(actual[1:], predicted[1:])

            if iou >= MATCH_IOU_THRESHOLD:
                candidates.append((predicted_index, iou))
                iou_lookup[(actual_index, predicted_index)] = iou

        adjacency[actual_index] = [
            predicted_index
            for predicted_index, _ in sorted(
                candidates,
    ...
    def assign(actual_index, visited_predictions):

        for predicted_index in adjacency[actual_index]:

            if predicted_index in visited_predictions:
                continue

            visited_predictions.add(predicted_index)
            previous_actual = predicted_to_actual.get(predicted_index)

            if previous_actual is None or assign(previous_actual, visited_predictions):
                predicted_to_actual[predicted_index] = actual_index
                return True

        return False

    for actual_index in adjacency:
        assign(actual_index, set())

    return sorted(
        (
            actual_index,
            predicted_index,
            iou_lookup[(actual_index, predicted_index)],
        )
        for predicted_index, actual_index in predicted_to_actual.items()
    )
...
def train_model(
    ...
    print()
    print("=" * 70)
    print("YOLO11n 최종 파인튜닝 시작")
    print("=" * 70)


    # --------------------------------------------------------
    # 기존 1차 학습 BEST 모델에서 파인튜닝
    # --------------------------------------------------------

    if not PRETRAINED_BEST_MODEL.exists():

        raise FileNotFoundError(
            "\n"
            "기존 1차 학습 best.pt를 찾을 수 없습니다.\n"
            f"확인 경로: {PRETRAINED_BEST_MODEL}\n"
            "PRETRAINED_BEST_MODEL 경로를 실제 best.pt 위치에 맞게 수정하세요."
        )

    print()
    print(
        f"Fine-tuning Base Model : "
        f"{PRETRAINED_BEST_MODEL}"
    )

    model = YOLO(
        str(PRETRAINED_BEST_MODEL)
    )
    ...
    results = model.train(
        ...
        # 기존 학습 모델을 미세 조정하므로 낮은 초기 학습률 사용
        lr0=0.001,
        ...
        # Validation 성능이 10 epoch 동안 개선되지 않으면 조기 종료
        patience=10,
    ...
    with open(
        ...
        for class_id, class_name in enumerate(
            ...
            try:
                ...
                print(
                    f"[WARNING] {class_name} metric CSV 저장 실패: {error}"
                )
    ...
    for index, image_path in enumerate(
        ...
        # ====================================================
            ...
            if 0 in actual_classes:
                ...
                # 이 부분은 이미지 단위 다중 레이블 평가입니다.
                # 박스 위치 정확도는 model.val() 탐지 지표와
                # correct_matches/image_has_miss로 별도 평가합니다.
                if 0 in predicted_classes:
            ...
            if predicted_is_background:
                ...
                result_text = (
                    "OBJECT -> OBJECT"
                    if spatial_object_detected
                    else "OBJECT -> OBJECT (NO SPATIAL MATCH)"
                )
...
def main():
    ...
    print()
    print("=" * 70)
    print("YOLO11n 화재 / 연기 / Background 1회 최종 파인튜닝")
    print("=" * 70)
