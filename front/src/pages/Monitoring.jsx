...
function Monitoring() {
  ...
  const requestedCctvStatus = searchParams.get('cctv_status');
  const filteredCCTVs = accessibleCCTVs.filter(cctv => {
    const matchesSearch = cctv.name.includes(searchQuery) ||
      cctv.id.includes(searchQuery) ||
      cctv.location?.includes(searchQuery);
    const matchesStatus = !requestedCctvStatus ||
      (requestedCctvStatus === 'UNHEALTHY' && cctv.status !== 'normal') ||
      (requestedCctvStatus === 'ACTIVE' && cctv.status === 'normal') ||
      (requestedCctvStatus === 'INACTIVE' && cctv.status === 'offline') ||
      (requestedCctvStatus === 'ERROR' && cctv.status === 'fire');
    return matchesSearch && matchesStatus;
  });

  const clearCctvStatusFilter = () => {
    const nextParams = new URLSearchParams(searchParams);
    nextParams.delete('cctv_status');
    setSearchParams(nextParams, { replace: true });
  };

  useEffect(() => {
    const requestedCctvNo = searchParams.get('cctv_no');
    if (!requestedCctvNo) return;

    const target = accessibleCCTVs.find((cctv) => String(cctv.cctv_no) === String(requestedCctvNo));
    if (target && String(selectedCCTV?.cctv_no) !== String(target.cctv_no)) {
      setSelectedCCTV(target);
    }
  }, [accessibleCCTVs, searchParams, selectedCCTV?.cctv_no]);
  ...
  useEffect(() => {
    ...
    handleOpenEventDetail(requestedEventNo);
          ...
          <div className="absolute top-4 left-4 z-30 flex flex-wrap items-center gap-2 pointer-events-none">
            <div className="flex items-center gap-2 pointer-events-auto">
              ...
              {requestedCctvStatus && (
                <button
                  type="button"
                  onClick={clearCctvStatusFilter}
                  className="px-3.5 h-[38px] rounded-full text-xs font-bold bg-primary text-on-primary flex items-center gap-1.5 cursor-pointer border border-primary focus:outline-none focus-visible:outline-none"
                  title="대시보드에서 적용된 CCTV 상태 필터 해제"
                >
                  <span>{requestedCctvStatus === 'UNHEALTHY' ? '점검 필요' : requestedCctvStatus} 필터</span>
